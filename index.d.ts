declare module "react-spreadsheet" {
  ...
  type OnSignatures<ListenersT> = {
    [EventT in keyof ListenersT]: (
      event: EventT,
      listener: ListenersT[EventT]
    ) => void;
  };
  type OnAll<ListenersT> = UnionToIntersection<
    OnSignatures<ListenersT>[keyof ListenersT]
  >;

  interface FormulaParserCoords {
    index: number;
  }
  ...
  interface FormulaParserEvents {
    callCellValue: (
      cellCoord: FormulaParserCell,
      done: FormulaParserDone
    ) => void;
    callRangeValue: (
      startCellCoord: FormulaParserCell,
      endCellCoord: FormulaParserCell,
      done: FormulaParserDone
    ) => void;
  }
  ...
  export function createEmptyMatrix<T = DefaultCellValue>(
    rows: number,
    columns: number
  ): Matrix<T>;
  ...
  > extends React.Component<SpreadsheetProps<Cell, Value>> {}
